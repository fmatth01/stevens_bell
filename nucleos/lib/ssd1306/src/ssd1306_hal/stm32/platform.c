...
// ── Hardware config ───────────────────────────────────────────────────────────
// I2C1 on PB6 (SCL) and PB7 (SDA), AF4, open-drain.
...
static void i2c_flush(void)
{
    ...
    I2C1->CR2 = ((uint32_t)chunk   << I2C_CR2_NBYTES_Pos)         |
                               ...
                               ssd1306_platform_i2cConfig_t *cfg)
{
    ...
    // PB6 and PB7: AF mode (MODER = 0b10).
    ...
    // No pull (SSD1306 modules have on-board pull-ups).
    GPIOB->PUPDR &= ~(GPIO_PUPDR_PUPD6 | GPIO_PUPDR_PUPD7);

    // AF4 (I2C1) for PB6 and PB7 — AFRL bits [27:24] and [31:28].
